...
# Left-side branding + input layout
# Stable Streamlit UI, no blank panels
# =========================================================
...
def to_word_text(df):
    lines = [
        ...
        "",
        "Model V1.5 | Input: Ash, C, H, O, N, S",
    ]
...
.top-line {
    height: 8px;
    border-radius: 999px;
    background: linear-gradient(90deg,#0757c8,#7bb7ff,#f4f8ff);
    margin-bottom: 18px;
}
.left-brand {
    border-radius: 20px;
    ...
    box-shadow: 0 8px 24px rgba(0,70,173,.08);
    margin-bottom: 20px;
}
.brand-wrap {
    display: flex;
...
with tab1:
    left, right = st.columns([1.0, 2.1], gap="large")
...
""",
        ...
        with st.form("single_input_form", border=True):
            ...
            if abs(total - 100) <= 1:
                st.markdown(
                    f'<div class="input-check-ok">Elemental total = {total:.2f}% &nbsp; | &nbsp; Qualified</div>',
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    f'<div class="input-check-warn">Elemental total = {total:.2f}% &nbsp; | &nbsp; Check input</div>',
                    unsafe_allow_html=True,
                )

            submitted = st.form_submit_button("▣  Calculate thermodynamic parameters")
...
<div class="result-grid">
  ...
  <div class="card check">
    <div class="card-label">Elemental Total</div>
...
st.markdown(
    """
<div class="footer">
  <b>Biothermodata</b> &nbsp; | &nbsp; Model V1.5 &nbsp; | &nbsp; © 2025 Nanjing Tech University
</div>
""",
    unsafe_allow_html=True,
)
