...
# =========================================================
# Biomass Thermodynamic Agent
# Thermodynamic calculator only
...
def calculate_biomass_thermo(data: BiomassInput) -> Dict[str, float]:
    ...
    # Biomass standard entropy, J/(kg·K)
    S_biomass_raw = (
        0.0055 * C
        + 0.0954 * H
        + 0.0096 * O
    ...
    # Combustion O2 coefficient
    o2 = x + y / 4.0 - z / 2.0 + b

    # Reaction entropy, J/(kg·K)
    dS_raw = (
        x * S_CO2
        + y / 2.0 * S_H2O_L
        + a / 2.0 * S_N2
...
def to_word_text(df):
    ...
    lines.append("")
    lines.append("Model V1.0 | Based on elemental analysis: Ash, C, H, O, N, S")
...
<style>
    ...
    .hero {
        background: linear-gradient(90deg, #ffffff 0%, #f2f7ff 52%, #ffffff 100%);
    ...
    .panel {
        ...
        padding: 20px;
        background: #ffffff;
        box-shadow: 0 8px 28px rgba(0, 70, 173, 0.06);
        min-height: 620px;
    }
    ...
    .footer {
        ...
        border-radius: 16px;
        padding: 18px 26px;
        background: linear-gradient(90deg,#f5f9ff,#ffffff,#f5f9ff);
        display: grid;
    ...
    .stButton button {
        background: linear-gradient(90deg, #0046ad, #0969da);
        color: white;
        border-radius: 10px;
        border: none;
        font-weight: 750;
    ...
    .stButton button:hover {
        background: linear-gradient(90deg, #00357f, #0058c8);
        color: white;
        border: none;
    }
...
with tab1:
    ...
    with left:
        ...
        if abs(total - 100) <= 1:
            st.markdown(
                f'<div class="status-ok">● Elemental total = {total:.2f}%. Data are within the acceptable range.</div>',
                unsafe_allow_html=True,
            )
            status_text = "Success"
        else:
            st.markdown(
                f'<div class="status-warn">● Elemental total = {total:.2f}%. Please check whether normalization is required.</div>',
                unsafe_allow_html=True,
            )
...
<div class="note">
<b>Note:</b><br>
All outputs are calculated from elemental analysis data and calibrated against the golden dataset containing 265 biomass cases.
</div>
""",
            unsafe_allow_html=True,
        )
    ...
    with right:
        ...
        st.markdown(
            f'<div class="panel-title">📊 2. Thermodynamic parameters <span style="float:right;font-size:14px;color:#5d6b89;font-weight:600;">◷ {now}</span></div>',
            unsafe_allow_html=True,
        )
...
<div class="result-grid">
  ...
  <div class="card blue">
    <div class="card-label">🌡️ ΔrH°</div>
  ...
  <div class="card dark">
    ...
    <div class="card-unit">Total = {total:.2f}%</div>
  </div>
</div>
""",
            unsafe_allow_html=True,
        ...
        with b2:
            st.download_button(
                label="⬇ Download result as Word",
                data=to_word_text(result_df),
                file_name="biomass_thermodynamic_result.doc",
        ...
        with b3:
            st.download_button(
                ...
                file_name="biomass_thermodynamic_result.csv",
                mime="text/csv",
            )
    ...
    if st.button("⚛ Calculate batch results", type="primary"):
        ...
        if missing_cols:
            ...
            for _, row in input_df.iterrows():
                ...
                total_row = float(row["Ash"]) + float(row["C"]) + float(row["H"]) + float(row["O"]) + float(row["N"]) + float(row["S"])
                results.append({**row.to_dict(), "Element Total (%)": round(total_row, 2), **calculated})
            ...
            with b2:
                st.download_button(
                    label="⬇ Download batch results as Word",
                    data=to_word_text(result_df),
                    file_name="biomass_thermodynamic_batch_results.doc",
                    mime="application/msword",
                )
            with b3:
                st.download_button(
                    ...
                    data=result_df.to_csv(index=False).encode("utf-8-sig"),
                    file_name="biomass_thermodynamic_batch_results.csv",
                    mime="text/csv",
                )
...
<div class="footer">
  ...
  <div class="footer-right">
    Based on elemental analysis: Ash, C, H, O, N, S<br>
    Outputs are calibrated to the golden dataset (265 cases)
  </div>
</div>
""",
    unsafe_allow_html=True,
